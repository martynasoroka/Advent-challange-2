    ...
    const movies = [
      ...
      {
        "adult": false,
        "backdrop_path": "/aEgUv1KzUGPleCUuHrFVNvsyBUx.jpg",
        "id": 1123093,
      ...
      },
      {
        ...
        "poster_path": "/aOuX4hhYm0vbHP7dR2JLk6BAcfn.jpg",
        "media_type": "movie",
        "genre_ids": [
          28,
          12,
    ...
    movies.forEach((key, index) => {
      ...
      if((movies[index].overview).length >200) {
        overview.innerHTML += `<span class="overview">${(movies[index].overview).slice(0,197)}...</span>`
      } else {
        overview.innerHTML += `<span class="overview">${(movies[index].overview).slice(0,200)}</span>`
      }

    })


    const list = document.querySelector("fotogalerie")
    ...
    movies.forEach((key, index) => {
      ...
      <div class="galerie">
        <div class="foto">
          <img src="https://www.themoviedb.org/t/p/original/${movies[index].backdrop_path}" alt="Title">
          <div class="description">
          ${movies[index].title}<br>
          <span class="overview">${movies[index].overview}</span>
        </div>
      </div>
      `})
